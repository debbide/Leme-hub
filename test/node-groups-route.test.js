...
test('node group routes expose CRUD endpoints', async () => {
  ...
  const coreManager = {
    ...
    getSettingsSnapshot: () => ({ groupSortOrder, nodeGroupAutoTestIntervalSec: 300, nodeGroupLatencyCache: { updatedAt: null, results: {} } }),
    getNodeGroupTestingSnapshot: () => ({ intervalSec: 300, latencyCache: { updatedAt: null, results: {} } }),
    reorderNodeGroups: async (order) => { groupSortOrder = order; nodeGroups = [...nodeGroups].sort((a, b) => order.indexOf(a.id) - order.indexOf(b.id)); return { groupSortOrder: order }; },
  ...
  const reordered = await routes['POST /api/node-groups/reorder']({ body: { order: ['g2', 'g1'] } });
